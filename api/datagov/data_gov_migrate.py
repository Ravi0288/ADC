...
def push_to_figshare(request):
    ...
    metadata_adc_links_file=os.path.join(BASE_DIR, agency+"\logs\\"+env+"\metadata_link.json")
    with open(metadata_adc_links_file, 'rb') as f:
        metadata_adc_links=json.load(f)
    print(metadata_adc_links)
    metadata_error_file=os.path.join(BASE_DIR, agency+"\logs\\"+env+"\metadata_error.txt")
    metadata_report_file=os.path.join(BASE_DIR, agency+"\logs\\"+env+"\metadata_report.txt")
